...
class EuclideanClustering {
  ...
  public:
    ...
    void GNSSCallback(const gps_imu_ekf::Gnss::ConstPtr& gnss_msg);
...
EuclideanClustering::EuclideanClustering() {
    ...
    pub_object_markerarray = nh.advertise<visualization_msgs::Marker>("/object_marker_array", 100);
}
...
void EuclideanClustering::GNSSCallback(const gps_imu_ekf::Gnss::ConstPtr& gnss_msg) {
    gnss_pose = *gnss_msg;
}
...
std::vector<pcl::PointCloud<pcl::PointXYZI>::Ptr> EuclideanClustering::euclidean_clustering() {
    ...
    euclidean_clustering_pcl.header.frame_id = "map";
...
void EuclideanClustering::CalcInfo(const pcl::PointCloud<pcl::PointXYZI>::ConstPtr& clusters) {
    ...
    // object_markerarray.markers.clear();
...
void EuclideanClustering::ObjectVisualization(double object_lat, double object_long, double size) {
    ...
    pub_object_markerarray.publish(object_marker);
